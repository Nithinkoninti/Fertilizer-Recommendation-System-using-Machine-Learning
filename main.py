...
def preprocess_data_tkinter():
    ...
    if os.path.exists(encoder_path):
        ...
        Y = le_target.transform(df["Recommended_Fertilizer"].astype(str))

    else:

        le_target = LabelEncoder()

        Y = le_target.fit_transform(df["Recommended_Fertilizer"].astype(str))
...
def predict_stacking_model_tkinter():
    ...
    results['Predicted_Fertilizer'] = y_pred_labels
...
def show_admin_buttons():
    ...
    tk.Button(main, text="Upload Dataset", command=upload_dataset, font=font1, bg="black", fg="white").place(x=20, y=100)

    tk.Button(main, text="Preprocess Dataset", command=preprocess_data_tkinter, font=font1, bg="black", fg="white").place(x=300, y=100)

    tk.Button(main, text="Train/Test Split", command=split_train_test_tkinter, font=font1, bg="black", fg="white").place(x=500, y=100)

    tk.Button(main, text="Perform EDA", command=perform_eda_tkinter, font=font1, bg="black", fg="white").place(x=700, y=100)

    tk.Button(main, text="Train LDA Model", command=lambda: train_lda_classifier_tkinter(X_train, y_train, X_test, y_test), font=font1, bg="black", fg="white").place(x=20, y=150)
    tk.Button(main, text="Train SVM Model", command=lambda: train_kernel_svm_classifier_tkinter(X_train, y_train, X_test, y_test), font=font1, bg="black", fg="white").place(x=300, y=150)
    tk.Button(main, text="Train QDA Model", command=lambda: train_qda_classifier_tkinter(X_train, y_train, X_test, y_test), font=font1, bg="black", fg="white").place(x=500, y=150)

    tk.Button(main, text="Train Stacking Model", command=lambda: train_bagging_logistic_stacking_tkinter(X_train, y_train, X_test, y_test), font=font1, bg="black", fg="white").place(x=700, y=150)

    tk.Button(main, text="Plot Model Performance", command=plot_model_performance_tkinter, font=font1, bg="black", fg="white").place(x=900, y=150)
...
def show_user_buttons():
    ...
    tk.Button(main, text="Upload Test Data", command=upload_test_data, font=font1, bg="white", fg="black").place(x=10, y=150)
    tk.Button(main, text="Predict (Stacking Model)", command=predict_stacking_model_tkinter, font=font1, bg="white", fg="black").place(x=150, y=150)
    tk.Button(main, text="View Model Performance Graph", command=plot_model_performance_tkinter, font=font1, bg="white", fg="black").place(x=350, y=150)

    tk.Button(main, text="Exit", command=close, font=font1).place(x=600, y=150)
